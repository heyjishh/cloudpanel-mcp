...
`);
  ...
  for (const platform of PLATFORMS) {
    ...
    if (onlyPlatforms.length > 0 && !onlyPlatforms.some(p => name.includes(p))) continue;
    ...
    try {
      ...
      if (platform.isNested) {
        if (!config[platform.mergeKey]) config[platform.mergeKey] = {};
