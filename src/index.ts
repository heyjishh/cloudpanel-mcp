...
import { z } from "zod";
import { SSHClient, SSHConfig } from "./ssh.js";
import fs from "fs";
...
server.tool(
  "cloudpanel_list_sites",
  "List all sites in CloudPanel",
  {},
  async () => {
    try {
      await ensureClpctl();
...
server.tool(
  ...
  async ({ domainName, phpVersion, siteUser, webTemplate }) => {
    try {
      ...
      if (siteUser) cmd += ` --siteUser=${siteUser}`;
      if (webTemplate) cmd += ` --vhostTemplate='${webTemplate}'`;
...
server.tool(
  ...
  async ({ domainName, nodeVersion, appPort, appPath }) => {
    try {
      ...
      if (appPath) cmd += ` --appPath=${appPath}`;
      const result = await ssh.runClpctl(cmd);
...
server.tool(
  "cloudpanel_create_python_site",
  "Create a Python (pipenv/wsgi) site via CloudPanel",
  {
    domainName: z.string().describe("Domain name"),
  ...
  async ({ domainName, pythonVersion, appPort, appPath, appType }) => {
    try {
      ...
      if (appPath) cmd += ` --appPath=${appPath}`;
      if (appType === "asgi") cmd += ` --appType=asgi`;
      const result = await ssh.runClpctl(cmd);
...
server.tool(
  "cloudpanel_create_reverse_proxy_site",
  "Create a reverse proxy site (for Docker containers)",
  {
    domainName: z.string().describe("Domain name"),
...
server.tool(
  "cloudpanel_delete_site",
  "Delete a site from CloudPanel",
  {
  ...
  async ({ domainName, deleteDirectory }) => {
    try {
      ...
      if (deleteDirectory) cmd += ` --deleteHomeDirectory`;
      const result = await ssh.runClpctl(cmd);
...
server.tool(
  "cloudpanel_install_ssl",
  "Install Let's Encrypt SSL certificate for a domain",
  {
    domainName: z.string().describe("Domain name"),
...
server.tool(
  "cloudpanel_list_ssl_certificates",
...
server.tool(
  "cloudpanel_create_mysql_database",
...
server.tool(
  ...
  async ({ userName, password, databaseName }) => {
    try {
      ...
      if (databaseName) cmd += ` --databaseName=${databaseName}`;
      const result = await ssh.runClpctl(cmd);
...
server.tool(
  "cloudpanel_delete_mysql_database",
...
server.tool(
  "cloudpanel_create_postgresql_database",
  "Create a PostgreSQL database (if available)",
  {
    databaseName: z.string().describe("Database name"),
  },
  async ({ databaseName }) => {
...
server.tool(
  "cloudpanel_system_info",
  "Get system information (OS, memory, disk, uptime, Docker version)",
  {},
  async () => {
    try {
...
server.tool(
  "cloudpanel_create_user",
  "Create a system user (for FTP/site access)",
  {
    userName: z.string().describe("Username"),
    password: z.string().describe("Password"),
  ...
  async ({ userName, password, homeDirectory }) => {
    try {
      ...
      if (homeDirectory) cmd += ` --homeDirectory=${homeDirectory}`;
      const result = await ssh.runClpctl(cmd);
      return { content: [{ type: "text", text: result.stdout || result.stderr || "User created" }] };
    } catch (e: any) {
      return { content: [{ type: "text", text: `Error: ${e.message}` }], isError: true };
...
server.tool(
  "cloudpanel_enable_2fa",
  "Enable two-factor authentication for a CloudPanel admin user",
  {
...
server.tool(
  "cloudpanel_disable_2fa",
  "Disable two-factor authentication for a CloudPanel admin user",
  {
    userName: z.string().default("admin").describe("Admin username"),
  },
  async ({ userName }) => {
...
server.tool(
  "cloudpanel_create_backup",
  "Create a CloudPanel backup",
  {
...
server.tool(
  "cloudpanel_list_backups",
  "List CloudPanel backups",
  {},
  async () => {
    try {
...
server.tool(
  "cloudpanel_delete_backup",
  "Delete a CloudPanel backup",
  {
...
server.tool(
  ...
  "Deploy a docker-compose.yml file on the server",
  {
...
server.tool(
  "docker_prune",
  "Remove unused Docker data (containers, images, volumes)",
  {
...
server.tool(
  "server_install_docker",
  "Install Docker Engine on the server. Supports multiple install methods.",
  {
  ...
  async ({ method, version, dryRun }) => {
    ...
    if (dryRun) {
      ...
      if (version) results.push(`   - Version pin: ${version}`);
    ...
    if (method === "apt") {
      const { stdout, stderr } = await ssh.exec(
        `DEBIAN_FRONTEND=noninteractive apt-get update -qq && DEBIAN_FRONTEND=noninteractive apt-get install -y -qq docker.io docker-compose-v2 2>&1 && docker --version`
      );
...
server.tool(
  "server_install_nodejs",
  "Install Node.js on the server via NodeSource or nvm.",
  {
...
server.tool(
  "server_software_status",
  "Check what software is installed and running on the server.",
  {
...
server.tool(
  "server_install_certbot",
  "Install Certbot (Let's Encrypt SSL client) with Nginx plugin.",
  {
    dryRun: z.boolean().default(false).describe("Preview without executing"),
  },
  async ({ dryRun }) => {
...
server.tool(
  ...
  async ({ version, rootPassword, dryRun }) => {
    ...
    if (dryRun) {
      ...
      if (rootPassword) results.push(`   - Set root password`);
...
server.tool(
  "deploy_autoforge",
  "Deploy the AutoForge project (autoaveriq.ca) to the server. Choose deployment method: docker (full stack with Postgres), cloudpanel-reverse-proxy (Docker apps behind CloudPanel proxy), or cloudpanel-nodejs (CloudPanel-managed Node.js site). Installs prerequisites automatically. Shows live progress.",
  {
  ...
  async ({ method, frontendDomain, backendDomain, secretKey, databaseUrl, dbPassword, autoSsl, runMigrations, skipBackup, startPort }) => {
    ...
    const results: string[] = [];
    const step = (msg: string) => { results.push(`\n▶ ${msg}`); };
...
server.tool(
  ...
  "Check firewall status and rules (UFW or iptables).",
  {
...
server.tool(
  "server_firewall_allow_port",
  "Allow a port through the firewall (UFW).",
  {
...
server.tool(
  "server_dns_check",
  "Check DNS resolution for a domain and verify it points to this server.",
  {
  ...
  async ({ domain }) => {
    ...
    if (cname.trim()) results.push(`📌 CNAME: ${cname.trim()}`);

    const { stdout: wwwRecord } = await ssh.exec(`dig +short www.${domain} A 2>/dev/null`);
    if (wwwRecord.trim()) results.push(`📌 www.${domain}: ${wwwRecord.trim()}`);
...
server.tool(
  ...
  "Execute any command on the server (escape hatch — use with caution)",
  {
